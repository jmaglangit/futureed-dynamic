angular.module('futureed.services')
	.factory('ManageTeacherReportsService', ManageTeacherReportsService);

ManageTeacherReportsService.$http = ['$http'];

function ManageTeacherReportsService($http) {
	var api = {};
	var apiUrl = '/api/';
	...
	api.getAllClass = function(id){
		return $http({
			method 	: 	Constants.METHOD_GET,
			url 	: 	apiUrl + 'v1/classroom?' 
						+ 'client_id=' + id
						+ '&payment_status=Paid'
		});
	}

	api.getAllStudentByClass = function(id){
		return $http({
			method 	: 	Constants.METHOD_GET,
			url 	: 	apiUrl + 'v1/classroom/' + id + '/students'
		});
	}
