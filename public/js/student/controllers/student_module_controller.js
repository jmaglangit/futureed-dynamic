	...
	self.dynamicQuestionSetup = function(question){
		...
		switch(question.question_template.operation){
			...
			case Constants.FRACTION_ADDITION_WHOLE:
				...
				question_text = question_text.replace("{fraction_addition_whole}","");
			...
			case Constants.FRACTION_ADDITION_BUTTERFLY:
				...
				setRandomDigits(self.question_grade_condition.max_number.toString().length);
				randomDigitsOnclick();
				question_text = question_text.replace("{fraction_addition_butterfly}", "");
			...
			case Constants.CONVERT_NUMBER:
				...
				randomDigitsOnclick();
				question_text = question_text.replace("{convert_integer}",getRandomNumber1());
