...
function generateCorrectAnswer(){
	arrAnswer = ['largest number'];
	arrYourAnswer = [];
	_arrNum = [];
	arrAnswer[1] = arrNum.length;
	arrAnswer[2] = arrNum.reduce(function(a, b) { return Math.max(a, b);});
	arrAnswer[3] = arrNum.reduce(function(a, b) { return Math.min(a, b);});
	_arrNum = arrNum.slice();
	_arrNum.sort(function(a, b){return b-a});
...
function generateAnswerFlow(){
	$("#lastDiv2").html('Use these integers and construct the largest number possible<br><span class="boldStr">'+randomNumber+'</span>');
    $("#lastDiv3").html('Use these integers and construct the largest number possible<br><span class="boldStr">'+randomNumber+'</span>');
	step_count = 0;
	for(real_step_count = 0; real_step_count < pos_last + 2; real_step_count++){
		...
		if(arrYourAnswer[real_step_count].toLowerCase() != arrAnswer[real_step_count]) {
			if (real_step_count == 0) {
				strHTML += "<p style='color:red;'> Largest number Error : " + arrYourAnswer[real_step_count] + "</p>";
			}else{
				strHTML += "<p style='color:red;'> Error : " + arrYourAnswer[real_step_count] + "</p>";
			}
		}		    	

		if(real_step_count == 0) strHTML += "<br><span>In this case we need to figure out the largest number.</span>";
			...
			strHTML += "<br><span class='toolTopSpan'>First rank your numbers from largest to smallest.</span>";
			strHTML += "<br><span>We know that "+_arrNum.join(">")+"</span>";
			strHTML += "<br><span class='toolTopSpan'>Your strategy when creating the largest number is to put the largest number on the far left.</span>";
  			strHTML += "<br><span>In this case the largest number is "+arrAnswer[real_step_count]+" since "+_arrNum.join(">")+"</span>";
		}
		else if(real_step_count == 3) strHTML += "<br><span class='toolTopSpan'>The smallest number goes on the far right, that is "+arrAnswer[real_step_count]+"</span>";
...
function validateAnswer4Sort(_elem, _correct_answer, __start_num, __end_num) {
	...
	if((_answer * 1 < __start_num) || (_answer * 1 > __end_num))	{
		if( retry_attempt > retry_attempt_limit ) {
			retry_attempt = 0;										return _errorHandler(_elem, -6, "Correct Answer is " + _correct_answer + ". Retry! ");
		}else{
    ...
    if(_answer < _correct_answer) {
    	if( retry_attempt > retry_attempt_limit ) {
			...
			return _errorHandler(_elem, -3, "opps not enough, your answer needs to be larger.");	
		}
