...
var numMax = 0;
var str_intro = "";
var randomNumber_decimals1 = "";
var randomNumber_decimals2 = "";

var lastBeforeErrortemp1 = 0;
var lastBeforeErrortemp2 = 0;
var lastBeforeErrortemp3 = 0;
var lastBeforeErrortemp4 = 0;
...
function getRandomNumber1(){
    return randomNumber_decimals1;
}

function getRandomNumber2(){
    return randomNumber_decimals2;
}
...
function randomDigitsOnclick(){
    ...
    lastBeforeErrortemp1 = 0;
    lastBeforeErrortemp2 = 0;
    lastBeforeErrortemp3 = 0;
    lastBeforeErrortemp4 = 0;
    total_result = 0;
    reallyTotal_result = 0;
    ...
    randomDigits1 = $("#randomDigits1").prop("value");
    randomDigits1 = parseInt(randomDigits1);
    if(isNaN(randomDigits1)) randomDigits1 = 1;

    randomDigits2 = $("#randomDigits2").prop("value");
    ...
    randomNumber_decimals1 = genRandomNumber(0.05, numMax, randomDigits1);
    randomNumber_decimals2 = genRandomNumber(0.05, numMax, randomDigits2);
    
    randomNumber1 = Math.floor(randomNumber_decimals1*digits(randomDigits1));
    randomNumber2 = Math.floor(randomNumber_decimals2*digits(randomDigits2));

    $("#randomNumber1").prop("value", randomNumber_decimals1);
    $("#randomNumber2").prop("value", randomNumber_decimals2);
    $("#subject_number1_p").html(randomNumber_decimals1);
    $("#subject_number2_p").html(randomNumber_decimals2);
...
function lastBeforecheckAnswerValidation1(elem) {
    answer_val = parseInt(elem.prop("value"));
    correct_answer = 0;
    correct_answer = randomDigits1;
    ...
    if (answer_val > correct_answer) {
        if(retry_attempt > 1){
            ...
            if (!lastBeforeErrortemp1) {
                lastBeforeErrortemp1 = answer_val;
            }
            return -1;
        }
    }else{
...
function lastBeforecheckAnswerValidation2(elem) {
    answer_val = parseInt(elem.prop("value"));
    correct_answer = 0;
    ...
    if (answer_val > correct_answer) {
        if(retry_attempt > 1){
            ...
            if (!lastBeforeErrortemp2) {
                lastBeforeErrortemp2 = answer_val;
            }
            return -1;
        }
    }else{
...
function lastBeforecheckAnswerValidation3(elem) {
    answer_val = parseInt(elem.prop("value"));
    correct_answer = 0;
    correct_answer = randomDigits2*1 + randomDigits1*1;
    ...
    if (answer_val > correct_answer) {
        if(retry_attempt > 1){
            ...
            if (!lastBeforeErrortemp3) {
                lastBeforeErrortemp3 = answer_val;
            }
            return -1;
        }
    }else{
...
function lastBeforecheckAnswerValidation4(elem) {
    answer_val = elem.prop("value");
    correct_answer = 0;
    correct_answer = total_result / digits(randomDigits1 + randomDigits2) + "";
    reallyTotal_result = correct_answer;
    if (answer_val == correct_answer){
        return correct_answer;  
    } 
    ...
    if (answer_val > correct_answer) {
        ...
        if(retry_attempt > 1){
            ...
            if (!lastBeforeErrortemp4) {
                lastBeforeErrortemp4 = answer_val;
            }
            return -1;
        }
    }else{
...
function IsfinishBefore(){
    ...
    $(str_before).insertBefore("#lastDiv");
    return IsfinishBeforeValidation();
}
...
function IsfinishBeforeValidation(){
    ...
    $(".inputCheck1").unbind("keydown").keydown(function(event){
        if(event.keyCode == 13){
            ...
            temp_answer = lastBeforecheckAnswerValidation1($(this));
            if(temp_answer == -1){
                alertModal("Your answer is larger than what we need.");
                $(this).prop("value", "").focus();
                retry_attempt++;
            ...
            $(".inputCheck2").unbind("keydown").keydown(function(event){
                if(event.keyCode == 13){
                    ...
                    temp_answer = lastBeforecheckAnswerValidation2($(this));
                    if(temp_answer == -1){
                        alertModal("Your answer is larger than what we need.");
                        $(this).prop("value", "").focus();
                        retry_attempt++;
                    ...
                    $(".inputCheck3").unbind("keydown").keydown(function(event){
                        if(event.keyCode == 13){
                            ...
                            temp_answer = lastBeforecheckAnswerValidation3($(this));
                            if(temp_answer == -1){
                                alertModal("Your answer is larger than what we need.");
                                $(this).prop("value", "").focus();
                                retry_attempt++;
                            ...
                            $(".inputCheck4").unbind("keydown").keydown(function(event){
                                if(event.keyCode == 13){
                                    ...
                                    temp_answer = lastBeforecheckAnswerValidation4($(this));
                                    if(temp_answer == -1){
                                        alertModal("Your answer is larger than what we need.");
                                        $(this).prop("value", "").focus();
                                        retry_attempt++;
...
function reallyEnd() {
    // body...
    $("<p>Result Value : </p><b>" + reallyTotal_result + "</b>").insertBefore("#lastDiv");
    answerDone();   //added
    displayTotalFlow();
    displayTotalFlow1();
}
...
function displayTotalFlow(){
    ...
    result += IsfinishBeforeShow1();
    if (lastBeforeErrortemp1) {
        result += "<p style='color:red;'> Error : " + lastBeforeErrortemp1 + "</p>";
    }
    if (lastBeforeErrortemp2) {
        result += "<p style='color:red;'> Error : " + lastBeforeErrortemp2 + "</p>";
    }
    if (lastBeforeErrortemp3) {
        result += "<p style='color:red;'> Error : " + lastBeforeErrortemp3 + "</p>";
    }
    if (lastBeforeErrortemp4) {
        result += "<p style='color:red;'> Error : " + lastBeforeErrortemp4 + "</p>";
    }
    result += "<p>Result Value : <font color=blue><b>" + reallyTotal_result + "</b></font></p>";

    var arry_numbertostr = [];
    str = reallyTotal_result.toString();
    var isDotFount = false;

    for (var i = 0; i < reallyTotal_result.toString().length; i++) {
        if( isDotFount == true)
            result += "<b style='position:relative; top:10px; left: 28px;color:red;'>&#10558</b>";
        if( i < str.length)
            arry_numbertostr[i] = str[i];
        
        result += "<label style='color:blue; font-size:25px; position:relative; left:14px;'>" + arry_numbertostr[i] + "</label>";
        if (str[i] == ".") {
            isDotFount = true;
        }
    }
...
function displayTotalFlow1(){
    ...
    var reallyTotal_result_correct = 0;
    reallyTotal_result_correct = (randomNumber1 * randomNumber2) / digits(randomDigits2 + randomDigits1);
    strHtml += "</p>";
    ...
    result += IsfinishBeforeShow2();
    result += "<p>Result Value : <font color=blue><b>" + reallyTotal_result_correct + "</b></font></p>";

    var arry_numbertostr = [];
    str = reallyTotal_result_correct.toString();
    var isDotFount = false;

    for (var i = 0; i < reallyTotal_result_correct.toString().length; i++) {
        if( isDotFount == true)
            result += "<b style='position:relative; top:10px;left: 28px;color:red;'>&#10558</b>";
        if( i < str.length)
            arry_numbertostr[i] = str[i];
